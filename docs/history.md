...
## A second listener cost one page or none, and which depended on two pointers

`listen(.{ .also = … })` looked like a loop around code that already existed, and the part that was not a loop is where the memory went. The acceptors' shared struct held both what a *listener* has (its certificate) and what the *server* has (the connection budget, the descriptor-shortage flag, the first failure), and a connection fiber keeps a pointer to it live across the handler. Splitting it the obvious way, by copying the server's three parts into each listener's struct, would have widened that frame — and [ADR 0288](./adr/0288-tls-is-an-option-a-build-asks-for.md) had already measured that the plain park sits under 300 bytes short of a page boundary, so anything at all crosses it. Two pointers instead of a struct kept it where it was: 9,300 bytes an idle connection before and 9,300 after, at ten thousand of them ([ADR 0289](./adr/0289-a-server-answers-on-more-than-one-address.md)).

**The per-listener figure was wrong by a third until it was measured.** An acceptor fiber's stack is 4 KB and there is one per thread, so an extra listener "obviously" costs 64 KB on sixteen threads, and that number went into the option's doc comment before anything was run. It is 82 KB. The first attempt to measure it compared one listener against two and got a delta inside its own run-to-run spread; comparing one against *five* and dividing by four put it beyond doubt in the same number of runs. **When a delta is inside the noise, multiply the thing you are measuring rather than the number of runs.**
...
## The line was drawn around a builder, and the applications drew it somewhere else

*One table, conditions that filter rows* was drawn before anything had been built on the module, and it held that joins and aggregates each break one of ADR 0171's two properties. Sorting every `db.raw` in the two applications built since put 63 of 272 statements at a join to one row and 104 at an aggregate, and neither breaks either property when the Row declares it instead of the call site: a reference points at one row, and a grouped Row's rows *are* its groups. What breaks them is a join to many, and the applications never wrote one as a join; they wrote `LATERAL` with `json_agg`, or N+1. **Read what callers write past a line before defending where it is**, because the property was right and the shape it was held against was a builder nobody proposed ([ADR 0295](./adr/0295-a-row-may-carry-its-parent-its-children-or-a-sum.md)).

The first build of children deadlocked the SQLite suite with no CPU: the second statement was sent from inside the reader of the first, whose result set was only drained on the way out. On a pool that is two connections held by one request; in a transaction it is one connection asked for two statements at once. **A read that issues another statement issues it after the first has let go**, and `ps -o etime,cputime` said so in one line.

## gRPC: the cost was the scheduler, and two of the bugs were not gRPC's

**A unary call's price was never HTTP/2.** The same build answers 3.12M calls a second with every call run on the connection's fiber and 0.77M with a fiber per call, and on one executor the fiber costs 0.23 µs. The rest is zio dealing every spawn to another executor and waking it. Inline is refused anyway, because a gRPC client multiplexes every call onto one connection; what closes the gap is a spawn homed on the calling executor, which is upstream's to add ([ADR 0297](./adr/0297-grpc-is-served-over-h2c-behind-a-flag.md), [`http.md`](../bench/result/http.md#a-grpc-listener-built)).

**`encoded.inflate` had refused a valid gzip body since it was written.** `std.compress.flate.Decompress` asks the writer for a byte of room before every block, the empty final block a flush-then-close writer ends with included, so a body exactly the size it announced failed with its last block unread. Go's writer ends that way and the Collector sends it on every call; an HTTP/1.1 body with `Content-Encoding: gzip` from the same writer failed the same way, and nothing in the suite had ever produced one.

**A pool held for 200 ms read as 5 MB.** Keeping a finished call's stream for the next call took allocations per call from 4 to 0, and made a burst of 10,000 connections hold 5 MB more heap, because every connection kept its spares until the idle release. Dropped when the connection waits with nothing in flight, the idle figure came out below the unpooled one.

**A fuzzer's generator is a test subject too.** The first frame generator produced 0 answered calls in 2,000 connections: SETTINGS values chosen at random sent almost every connection away at its second frame, so the properties held over refusals and nothing else. A test that counts answered calls, failed calls and GOAWAYs over a fixed seed now fails if that happens again. Reworked, the fuzzer's first run found DATA on a closed stream drawing an `RST_STREAM` for every frame a client sent.

**And a benchmark row was a server nobody had started.** The spike server gained a TLS listener whose certificate path is relative to the repository root, the throughput script started it from elsewhere, and it exited at once; its first row read 908k, from a server left over from an earlier session on the same port, and every later row read 0. The script now refuses to run beside one.
