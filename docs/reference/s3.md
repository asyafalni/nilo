...
| `bucket.list(c, .{ .prefix, .max_keys, .cursor })` | `Page` — `objects` (`Listed`: `key`, `size`, `etag`, `last_modified`) and `next`, the cursor for the page after or null. One page, at most 1,000; nothing follows the cursor for the caller ([ADR 0250](../adr/0250-a-list-is-a-page-with-a-cursor-and-nothing-that-follows-it.md)) |
