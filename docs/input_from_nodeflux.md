...
| 4, 6 | Done, [ADR 0222](./adr/0222-a-foreign-key-is-columns-and-a-table-name.md). A `.references` is a list of columns and a target, and the target is a Row **or the table's name**. The type check was not given up for the name: it moved to `orderOf`, which every path into the tool already goes through and where every Row is in one comptime list, so a table no Row claims is still an `@compileError`. A composite key is written as a table constraint and a one-column key stays inline, so every table generated before this is byte for byte the same. `.exists` joins on every column of the key. Five new Refusals. **`Reference.column` is `columns` and `target` is `targets`, which breaks the snapshot** the way `.key` → `.keys` did: `db generate` rewrites it, and a test holds the refusal of an older one. |
| 7 | Done, [ADR 0223](./adr/0223-a-version-file-is-a-generated-block-and-the-rest.md). `generate` writes the wrap the port was re-applying by hand: a `generated` decl between two marker lines, a `before` and an `after` for steps of your own, and `.steps = before ++ generated ++ after`. `--baseline` ignores the snapshot, derives version 1 from nothing and rewrites it in place, keeping every byte outside the block; it refuses when the directory has moved past version 1, when `--name` disagrees with the version 1 on disk, and when the markers are gone. **`before` is the answer to the ordering note** — an extension a generated column's type comes from now has somewhere to go. |
| `.check`, `.trigger`, `sql.Schema` | Decided in principle by ADR 0221, which names the second kind of word and builds none of it. What is outstanding is the list of object kinds whose replace is mechanical, which is what closes the kind. `nilo_sql`'s Next 1. |
| 10 | Not decided. |
