...
[ADR 0182](0182-a-leaf-that-says-what-it-is-can-be-carried.md) drew a line:
...
which is where [ADR 0181](0181-a-field-name-is-a-spelling-too.md)'s refusal
of `rename_all` fires.
...
caller sees — ADR 0181's rule applied one level down rather than relaxed.
...
neither. The line ADR 0182 drew stands; this adds the second thing a type
can say.
...
every marker does (ADR 0042: `http/` may not import `sql/`), so nothing about
...
- **Throughput: a saving where it applies.** A response holding a `Json(T)`
  ...
  Not measured on its own; the shape is the one ADR 0182 measured, 250 → 165ns
