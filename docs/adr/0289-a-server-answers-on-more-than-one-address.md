# 0289 — a server answers on more than one address

**Status:** accepted
**Applies:** [ADR 0018](./0018-the-trade-budget-has-three-axes.md)
(what a feature spends, and the number), [ADR 0273](./0273-every-executor-accepts.md)
(an acceptor per executor), [ADR 0288](./0288-tls-is-an-option-a-build-asks-for.md)
(one certificate per listener)
**Extends:** [ADR 0130](./0130-a-path-is-an-address-to-listen-on.md)
(a path is an address), [ADR 0002](./0002-the-engine-is-behind-a-seam.md)
...
one socket file behind it ([ADR 0028](./0028-what-nilo-will-not-do.md)).

[ADR 0288](./0288-tls-is-an-option-a-build-asks-for.md) changed what a
...
file. ADR 0288 already says "one certificate per listener" and "a listener
serving two names needs two listeners", which names a thing that could not
exist. ADR 0130 already made `address` carry a path as well as an IP, so
...
**An entry carries three fields, not the other thirty.** `Listener` is
`address`, `port` and `tls`. Everything else in `Options` is the *server's*
...
`Bound` the loop carries. Both are measured; the ADR 0018 table takes the
...
handed a `*Accepting` and keeps it live across the handler, and ADR 0288
...
API to buy tidiness for a field almost nobody sets. ADR 0288's own reasoning
...
`tlsRefusal` is (ADR 0288).

## What this does not do

- **No SNI, and still one certificate per listener.** Two names on one
  certificate, or two listeners, which is now a thing that exists. ADR 0288's
  row stands.
