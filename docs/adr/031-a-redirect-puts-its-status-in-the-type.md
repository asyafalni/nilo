...
ADR 023 already settled the shape of this trade for statuses generally: `Response(T)` picks its status while the request runs and the document can only write `default`; `Status(code, T)` puts it in the signature and the document names it.
...
The shape this exists to serve is: read a form, start a session, set a cookie, send them back to the page. Three of those four are a redirect's business, so `Redirect` has the same `headers` field `Response` and `Status` do, copied on the way out the same way (ADR 018).
