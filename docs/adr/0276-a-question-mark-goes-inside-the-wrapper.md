# 0276 — a `?` goes inside the wrapper, and outside it is refused

**Status:** accepted
**Applies:** [ADR 0024](./0024-a-failure-mode-belongs-in-the-return-type.md)
(`?T` is a 404), [ADR 0027](./0027-the-rule-about-error-messages-is-held-by-a-build-step.md)
(a mistake stops in nilo's own words)
**Extends:** [ADR 0258](./0258-a-version-a-handler-names-is-an-etag.md), which
...
- `?Status(code, T)` and `?Response(T)`: "the `?` has to go inside the
  ...
  everywhere else (ADR 0024). The document describes both.
...
- `?Versioned(T)`: "a thing that is not there has no version", which is
  ADR 0258's reason for refusing `Versioned(?T)`, applied one level up.
...
reader never wrote, which ADR 0027 is against.
