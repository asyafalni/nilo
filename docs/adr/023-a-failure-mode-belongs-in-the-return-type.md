...
[ADR 016](./016-the-api-description-comes-from-the-signatures.md) says the API description is read off the signatures, and [ADR 014](./014-what-nilo-borrows-and-from-whom.md) puts the ambition plainly: *the signature is the whole contract*. Building a CRUD app end to end is where that turns out not to be true yet.
...
- ADR 017 is untouched: this is all comptime data and a branch that compiles away. Nothing allocates and no per-request work is added.
