# 0247 — a route can say "cache this answer for a minute"

**Status:** accepted
**Extends:** [ADR 0193](./0193-a-request-answered-once-is-answered-the-same-way-again.md), whose record, marker and Space shape this reuses.
...
nothing that waits may go inside it ([ADR 0138](./0138-a-cache-holds-its-bytes-under-a-lock-it-can-spin-on.md)),
...
Everything else is ADR 0193's. The record is `idempotent.zig`'s, used and
...
- **Memory per idle connection.** None. Nothing is on the stack — the
  ...
  ([ADR 0063](./0063-a-handlers-stack-is-per-connection.md)). A request
...
it in that order — which is the argument ADR 0193 made and it holds here
...
thing [ADR 0041](./0041-a-module-sits-where-the-loop-puts-it.md) exists
