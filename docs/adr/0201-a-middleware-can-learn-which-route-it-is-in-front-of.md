...
The port that asked for [ADR 0149](0149-a-route-can-say-its-own-name.md)
...
- **Allocations per request: zero.** The name is on the `Route`, and the `Ctx`
  ...
  already carrying for metrics ([ADR 0100](0100-the-route-table-is-the-registry.md)),
...
- **Memory per idle connection: 4,669 bytes, unchanged.** The pointer is eight
  ...
  before the connection waits ([ADR 0071](0071-where-a-connection-waits-is-what-it-costs.md)).
  **A handler that parks its frame pays eight bytes per connection** — a
  WebSocket, a stream nobody closes — by the rule of
  [ADR 0063](0063-a-handlers-stack-is-per-connection.md), on top of the
