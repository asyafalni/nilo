# A message that arrived whole is handed over where it lies

**Status:** accepted
**Topic:** [websocket](../design/websocket.md)
**Applies:** [ADR 046](./046-a-message-is-copied-once-and-framed-once.md) (a message is copied once), [ADR 062](./062-where-a-connection-waits-is-what-it-costs.md) (the message buffer belongs to the executor, not to the socket), [ADR 202](./202-a-reset-between-frames-is-a-client-that-has-gone.md) (the last time `echo-ws-limited` found something).
...
It keeps the loan `Message` already described. `data` is good until the next `receive`, and the read buffer is refilled only by the next `receive`, which is the same promise the message buffer made because the next message overwrote it anyway. Unmasking stays one pass (ADR 046): it XORs where the bytes lie instead of on the way out of the read buffer.
...
Against ADR 017's axes, `21890c6` against the change, the arena's own entry built from each, pinned to 4 cores (8 threads) with gcannon on the other 4, interleaved:
