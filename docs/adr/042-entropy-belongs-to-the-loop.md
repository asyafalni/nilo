...
[ADR 038](./038-a-module-sits-where-the-loop-puts-it.md) shipped
...
[ADR 041](./041-core-knows-what-time-it-is.md) settled the millisecond by
...
[ADR 013](./013-handlers-must-not-block-the-thread.md)). Going through the
...
([ADR 013](./013-handlers-must-not-block-the-thread.md)). Being
reachable only from a `Ctx` is what says *this call costs a wait, and here is
where the wait is paid for*.

**A `Run` gets nothing, and that is the interesting half.** ADR 038 left this
...
plain `zig test`, which ADR 038 made the entry condition for the layer.
...
- **`nilo_s3` is not unblocked**, and it was wrong to say the two waited on the
  ...
  its own decision (ADR 038's last consequence).
