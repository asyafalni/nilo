# 0231 — a header std owns goes out once

**Status:** accepted
**Amends:** the `Begin.user_agent` slot added for fdm under 0.4.0, which
made one of six reachable and left the caller to route the other five.
...
[ADR 0063](./0063-a-handlers-stack-is-per-connection.md) that is bytes on
