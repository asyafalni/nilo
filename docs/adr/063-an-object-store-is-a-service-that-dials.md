# An object store is a Service that dials

**Status:** accepted
**Topic:** [s3](../design/s3.md)
**Carries out:** [ADR 058](./058-most-of-an-s3-client-is-not-s3.md),
[ADR 059](./059-a-bucket-is-a-type-and-a-key-is-not.md),
[ADR 060](./060-a-signing-key-changes-once-a-day.md),
[ADR 061](./061-a-fitting-borrows-the-loop.md)

## Context

Three ADRs designed `nilo_s3` before a line of it existed: what belongs in it
(ADR 058), what a bucket and a key are (ADR 059), and how signing works (ADR 060). A
fourth built the layer it would sit on (ADR 061). This is what happened when they
were carried out, and it exists for the two things that could not be decided on
paper.

The first is a contradiction the design left standing. ADR 058 says *"the HTTP
is `std.http.Client`"* and puts the module's dependency count at zero.
ADR 061's Consequences say *"`nilo_s3` will import `nilo_fetch` rather than
...
is the right size.** ADR 061 argued a Fitting into existence on 65 lines of
policy and one caller. `nilo_s3` is the second caller, and the second caller is
what the layer was for.

## Decision

**`nilo_s3` imports `nilo_fetch`, and that is not a departure from ADR 058 —
it is ADR 058 read through ADR 061.** `nilo_fetch` *is* `std.http.Client`,
...
[ADR 057](./057-percent-is-needed-by-two-layers.md) was written to avoid one
layer down and the outcome ADR 061 was built to avoid at this one.

The dependency count ADR 058 was protecting is unchanged in the sense it
...
evidence available that ADR 061 cut in the right place. Had `nilo_s3` needed
...
[ADR 062](./062-where-a-connection-waits-is-what-it-costs.md) is 3 KiB on every idle
...
| **Memory per idle connection** | **0 B** | 4,670 B with `nilo_s3` linked and a `Store` held, against 4,669 for the framework without it. Holding a store costs the connection nothing. **This is a floor, not a total**: by [ADR 062](./062-where-a-connection-waits-is-what-it-costs.md) a handler suspended inside a `get` holds its stack at the high-water mark, and that number has not been taken. |
...
| **Binary size** | **not taken** | the two scratch programs for the stripped `ReleaseFast` A/B are written and unrun, so ADR 017's running total is not yet updated. |
...
**Write the HTTP inside `s3/`.** ADR 058 priced this at ~815 non-S3 lines, of
...
definition of a Service under ADR 061, and the entry condition follows it: a
...
**Ship `LIST` in v1.** Refused by ADR 059 and unchanged by building the rest:
...
- **ADR 061's open question is half-answered.** It said *"the second Fitting
