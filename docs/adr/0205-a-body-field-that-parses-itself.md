...
[ADR 0158](0158-one-arrival-one-answer.md)'s sentence — *one arrival has one
...
hand — `uuidOf`, the helper ADR 0158 deleted from `activity`, back one arrival
...
same wall [ADR 0085](0085-a-type-says-how-its-json-is-spelled.md) met for a
...
- `sql.Uuid` and `sql.Timestamp` carry `jsonParse` and `jsonParseFromValue`
  ...
  (ADR 0042), and both are twenty lines.
...
integer ([ADR 0206](0206-a-whole-number-inside-a-range-is-a-type.md)) is one
...
ADR 0142 said the 400 for a type nilo did not write "names the type and stops
