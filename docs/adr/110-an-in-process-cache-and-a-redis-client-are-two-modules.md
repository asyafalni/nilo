...
[ADR 038](038-a-module-sits-where-the-loop-puts-it.md) decides a module's
...
- **Session storage.** `http/session.zig` seals the whole session into a
  ...
  ([ADR 033](033-a-session-is-sealed-into-the-cookie.md)).
- **Rate limiting.** `http/allowance.zig` is a fixed table in this process
  ([ADR 092](092-an-allowance-is-a-table-sized-while-compiling.md)).
...
([ADR 036](036-the-shape-of-a-query-is-settled-while-compiling.md)). `cache`
...
[ADR 109](109-a-cache-holds-its-bytes-under-a-lock-it-can-spin-on.md) is what
