...
([ADR 077](077-a-completion-the-loop-holds-outlives-the-frame-that-submitted-it.md))
...
same JSON body every other refusal has ([ADR 024](024-every-failure-answers-as-json.md)).
...
slot. `.text` +608, `.rodata` +271. The row is in [ADR 017](017-the-trade-budget-has-four-axes.md).
...
tab ADR 077 refused to make a shutdown wait for, and the same argument reads
the same way here.
