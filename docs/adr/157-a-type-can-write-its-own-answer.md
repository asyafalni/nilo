...
[ADR 016](016-the-api-description-comes-from-the-signatures.md)).
...
says nothing — the same rule ADR 016 set for a type that writes its own JSON:
...
A kept answer ([ADR 155](155-a-request-answered-once-is-answered-the-same-way-again.md))
...
of them is a dependency or a design, and ADR 017 prices both.

**Content negotiation** — one type, several encodings, chosen by `Accept`.
[ADR 024](024-every-failure-answers-as-json.md) already
