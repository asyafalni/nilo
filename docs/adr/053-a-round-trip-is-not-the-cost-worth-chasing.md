...
[ADR 013](./013-handlers-must-not-block-the-thread.md) names and the
...
- **pg.zig has no pipelining API**, and adding one means Parse/Bind/Execute
  ...
  ([ADR 036](./036-the-shape-of-a-query-is-settled-while-compiling.md)) and
...
- **The shape that actually matters is already one round trip.** Many rows in
  ...
  ([ADR 047](./047-a-batch-is-one-array-per-column.md)), and that is what a
