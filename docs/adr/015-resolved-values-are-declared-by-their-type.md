...
ADR 008 shipped middleware with one gap written into its consequences:
...
3. **It costs every request something.** A map on `Ctx` is a field, an init, and a lookup on requests that never use it. Declaring the value keeps that cost on the routes that asked (ADR 017).

Elysia's `resolve` is where the shape came from (ADR 014). Zig can do it better than Elysia can, because the check happens before the program runs rather than during startup codegen.
...
- A handler taking a resolved value is still an ordinary function: `me(.{ .id = 7 })` in a test, no server, no fake request. That is the property ADR 002 exists to protect, and this does not spend any of it.
