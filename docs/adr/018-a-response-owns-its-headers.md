...
- A `Response(T)` is 264 bytes larger than its payload. If that ever shows up in a measurement, the cap is the knob, and ADR 017's allocation and per-connection invariants are both untouched — nothing here allocates.
- ADR 017's fourth axis (binary size) is unaffected: the array replaces a slice, and `of` inlines to a handful of stores.
