...
Three shapes, and the rule has to be right about each. **A sized body** gets `grace + announced / rate`: `readSizedBody` takes its 4 KiB step before committing the rest, so a client that announces a megabyte and goes quiet is refused in ten seconds rather than in the megabyte's worth of time it never earned. **A chunked body** announces nothing, so it is sized from `max_body`, the most it is allowed to be; a flat number was rejected because it would make chunked either the cheap way to hold a connection or the framing that cannot upload anything large, and sizing it this way gives it exactly the worst case of a body that announced `max_body`, so neither framing is the better attack. **A body nobody reads**, the rest of one thrown away after the handler so the connection can carry the next request, gets the same deadline, sized from what is left at most: the announced length, or `max_body` for a chunked body, or what a `bodyStream` may still send. It once kept the per-read limit, so a POST to a 404 dribbled at a byte every 29 seconds held its fiber with no end, for a body nothing was going to read; `test "a body the handler never read is thrown away against a rate floor, not read by read"` holds it. **A trailer section** is bounded in bytes too, `http1.max_trailer_bytes` (8 KiB): a line was bounded by the read buffer and the number of lines by nothing. **Everything else that is not a buffered body** (`bodyStream` while the handler reads it, a WebSocket, a held-open stream, the connection's own reads) keeps the per-read limit only: those are [ADR 019](./019-a-request-that-lasts-is-still-one-request.md)'s "a request that lasts is still one request", and none of them is the framework holding memory on the client's behalf.
