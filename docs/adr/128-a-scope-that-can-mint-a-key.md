...
request sharing that thread (ADR 042). Being reachable only from a `Ctx` is
...
handler holds the thread and nothing in the log says so (ADR 044). A seed
...
- One field, one constructor and one method on `core.Run`. `core/` still names
  ...
  whole module with no build graph (ADR 038).
