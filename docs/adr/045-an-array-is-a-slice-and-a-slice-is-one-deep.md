...
which is the failure mode ADR 026 exists to keep out of this project.
...
one (ADR 007). This is the same fix as the enum column's, one layer over.
...
- `postgres.zig` reaches past pg.zig's API for the second time, after
  ...
  are in the one file allowed to know pg.zig exists (ADR 036).
...
- `insertMany` is now buildable, and is
  [ADR 047](./047-a-batch-is-one-array-per-column.md).
