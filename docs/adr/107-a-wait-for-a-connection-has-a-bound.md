...
[that is what the database is](065-one-writer-is-not-a-setting-it-is-the-database.md).
...
and it is [ADR 056](056-the-way-out-was-open-the-clock-was-not.md): the
...
([ADR 065](065-one-writer-is-not-a-setting-it-is-the-database.md)).
...
**The guess sent a reader the wrong way, and the line now names the holder instead.** An application's `/healthz` failed with the writer's message all through a thirty-second report, and its author went looking for a `tx` they did not have. The report ran on a reader. What held the writer was the previous probe's own `SELECT 1`: it had taken the connection and then queued for a thread in the Engine's pool, which zio's `blockInPlace` did not start for it while the report occupied the only worker ([zio#745](https://github.com/lalinsky/zio/issues/745)). So each connection keeps the text of the statement that took it, set under the lock that already guards `busy`, and the line reads *it is held by `SELECT 1`*; the reader's line names all of them. That tells the three cases apart without the fiber identity this ADR could not get: a `BEGIN` is a transaction, a long `SELECT` is the work itself, and a one-line statement still holding the connection after `timeout_ms` is waiting for a thread rather than for the database. The text is the holder's own and alive while it holds the connection, so nothing is copied, and it costs a slice on each pooled connection, per pool rather than per HTTP connection. Storing when it was taken as well, so the line could say for how long, was built and measured: a clock read per take was 4.5% of a prepared `find`, three pairs out of three, and the text alone is inside the noise ([`sql.md` §13](../../bench/result/sql.md#13-what-naming-a-connections-holder-costs-a-statement)). The waiter's own bound already says the holder has had it at least that long.
...
[ADR 032](032-a-guard-is-not-a-guard-until-it-has-been-seen-to-fail.md) that
...
([ADR 066](066-a-lazy-dependency-is-a-request.md)).

**It coordinates no port.** The server asks for port 0 and nothing connects: the
work under test is a spawned fiber (ADR 028), so the three loopback ranges the
...
[ADR 062](062-where-a-connection-waits-is-what-it-costs.md) stack is held per
