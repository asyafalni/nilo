# A service's wait on its own socket is a park

**Status:** accepted
**Topic:** [deadlines](../design/deadlines.md)
**Applies:** [ADR 013](./013-handlers-must-not-block-the-thread.md)
(a handler that holds its thread is noticed and named),
[ADR 013](./013-handlers-must-not-block-the-thread.md)
(what is measured is one unparked stretch),
[ADR 056](./056-the-way-out-was-open-the-clock-was-not.md)
...
stream, a WebSocket (ADR 013). A service that waits through its own `Io`
...
The report is the one that finds the invisible bug (ADR 013). A false
