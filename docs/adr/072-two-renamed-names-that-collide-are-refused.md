# Two renamed names that collide are refused
...
**Two refusal files rather than one** (ADR 026) — `refusals/`'s
...
refusal behind it is a branch that has only been seen to pass (ADR 032). The
...
This is the shape ADR 034 argues about ceilings: the loud answer is the one
worth having.
...
ADR 071 was, on `hello`, `rest`, `orders` and `forms` against a `git archive`
rebuild of `HEAD`.

**Compile time is the axis this spends, and it is the refusals that pay it.**
Two more files in `refusals/`, which never cache (ADR 026), on a step that is
