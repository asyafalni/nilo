...
[ADR 015](./015-resolved-values-are-declared-by-their-type.md) settled how a
...
Passing it explicitly at sixty call sites is what the port's own ADR 071
rejected. Wrapping the Scope is `pgtest.Scope` — the shim
[ADR 128](./128-a-scope-that-can-mint-a-key.md) deleted three days earlier —
...
carries `nilo_resolve` and is worked out from the request itself — so ADR 015's
...
**Not a type map on the request**, which ADR 015 rejected on three counts and
...
ADR 015's three counts, against this:
...
resolver, which is ADR 015's first objection walking back in through a door
