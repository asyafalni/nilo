...
([ADR 157](157-a-type-can-write-its-own-answer.md)). So the port took a
`*Ctx` and called `c.send(status, ct, bytes)` — and the generated document
said the route wrote something it could not read
([ADR 120](120-a-ctx-handler-that-returns-nothing-may-have-written-it.md)),
...
may have written anything, and ADR 120 is why the document declines to
