# A head is mostly cookies, and sixteen kilobytes of them
...
parsed in place (ADR 085), the ceiling on a request head: one that does not
...
**Memory per idle connection: unchanged, 4,669 bytes.** ADR 062 hands the
...
buffer into the arena. It is the parser's zero-copy property (ADR 085) and
