# 0260 — verified claims are a handler argument
...
it, and [ADR 0255](./0255-a-key-set-is-swapped-whole-and-freed-after-its-readers.md),
...
`job.Table(Db)` already gave for a store (ADR 0198) and `Keyring.refresh`
...
**The document says bearer.** The `security` entry ADR 0191 writes for an
...
0042, and rejected for the reason `nilo_http` does not import
`nilo_cache` (ADR 0138): a marker the caller's type carries needs no
