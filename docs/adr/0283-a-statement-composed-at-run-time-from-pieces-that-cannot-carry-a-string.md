...
[ADR 0148](0148-a-raw-statement-is-counted-while-compiling.md) made `db.raw`'s
...
[ADR 0204](0204-an-order-chosen-at-run-time-from-a-closed-set.md) then read
the property more carefully than the sentence: what ADR 0039 protects is
...
That is ADR 0204's property with one more kind of piece: a name that is
...
- **`param(n)` writes the `n`th placeholder spelled for the dialect** — `$n`
  ...
  ([ADR 0278](0278-a-raw-placeholder-is-spelled-for-the-dialect.md)). A
...
- **`db.composed(Row, c, stmt, values)`** and `tx.composed` run it, and
  ...
  check ([ADR 0134](0134-a-select-list-shorter-than-the-row-is-refused.md))
  holds; the values are converted the way a Row's are
  ([ADR 0145](0145-a-raw-parameter-is-converted-the-way-a-rows-is.md)); one
  column into a scalar works ([ADR 0234](0234-a-scalar-out-of-raw.md)). And
...
The two things ADR 0148 bought for `raw`, because both need the text while
...
[ADR 0057](0057-a-statement-that-is-a-constant-can-be-prepared-once.md)
...
is not the cost worth chasing; a caller that finds it is has ADR 0148's
answer, a finite set.

Nothing changes for `db.raw`: it keeps its comptime text, its column check
and its plan name, and ADR 0148's reasoning about it stands. This is not the
...
Cost on the axes of [ADR 0018](0018-the-trade-budget-has-three-axes.md): no
...
- **`db.rawText(Row, c, text, values)`** — the old signature under a new
  name. ADR 0148 already refused it, and it would refuse it again: a call that
