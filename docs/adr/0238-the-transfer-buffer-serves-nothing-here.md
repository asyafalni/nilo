# 0238 — the transfer buffer serves nothing here

**Status:** accepted
**Amends:** the `Begin.transfer_buffer` doc comment, the fetch guide's
"bigger is fewer trips", the S3 guide's "the buffer is yours because its
cost is yours", and lever 2 in `bench/result/fetch.md`.
...
| Memory per idle connection | `/call` at 5,000 connections moved from 9,450 to 9,437 bytes, interleaved twice, with ADR 0237's 64 bytes of `Exchange` in the same diff: inside the spread. **The 4 KiB was never a resident page**: a stack buffer no byte touches is never faulted in, and ADR 0063's per-connection figure counts pages. So lever 2 of `bench/result/fetch.md` (shrink the buffers) is measured now, and worth nothing |
