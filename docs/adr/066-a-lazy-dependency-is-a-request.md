# A lazy dependency is a request, not a conditional

**Status:** accepted
**Topic:** build-dependencies
**Extends:** [ADR 037](./037-a-service-that-needs-the-loop-is-finished-when-the-loop-exists.md)
...
against everything `refusals/` exists for (ADR 026).
...
ADR 038 already describes — the repository is modules — and it is the right
...
running something rather than by reading — `connect_on_init` (ADR 115), the
per-connection figure (ADR 062), three modules blocked on an open seam
(ADR 037), the page a connection held for nothing (ADR 062), and this. The
