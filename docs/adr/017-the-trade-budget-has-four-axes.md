...
| zio from v0.18.0 to `0299e57` on its `main`, built pinned ([ADR 199](./199-a-connection-is-served-by-the-thread-it-was-dealt-to.md)); zio's own code, and `spawnLocal` ([ADR 220](./220-grpc-is-served-over-h2c-behind-a-flag.md)) is +0 on both because only a `-Dgrpc` build calls it | +6,888 B | +6,904 B |
