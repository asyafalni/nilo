...
**Applies:** [ADR 021](./021-a-websocket-is-a-handler-that-does-not-return.md)
...
connection under `-r` with `SO_LINGER` at zero. The server after ADR 200
...
`Socket.receive` returns `null` when the client has gone. ADR 021 made
...
misreport ADR 022 removed from the write side.
...
ADR 200 alone made this column worse by half, and that was found by
...
interleaved pairs, `0efa4c0` → after (this and ADR 201):
...
- The rule to carry forward is ADR 062's, sharpened: **a `std.log` call
