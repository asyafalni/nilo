...
- **Fiber's best-known footgun is the one v1 closed.** `c.Params()` returns text pointing into a buffer that the next request reuses, and the rule saying so lives in the documentation. `Str` and `keep` (ADR 003) put that rule in the type system. Fiber gave up where nilo did the work.
...
This is what ADR 017's per-axis budget is made of.
...
- What this ADR does *not* decide: streaming responses, bodies over 1 MB, WebSocket and SSE, range requests and `sendfile`. Those are engine-shaped rather than DX-shaped, they each need the request arena to stop being the only answer to where memory comes from, and none of them was waiting on a philosophy. Each shipped under an ADR of its own: see the [responses](../design/responses.md), [websocket](../design/websocket.md) and [static-files](../design/static-files.md) topic pages.
