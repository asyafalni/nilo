# 0274 — a response is flushed before the connection waits, not before `send` returns

**Status:** accepted
**Refines:** [ADR 0009](./0009-middleware-is-an-onion-of-ctx-functions.md)
(the flush-on-send model).
**Applies:** [ADR 0002](./0002-zio-as-the-engine-behind-the-bulkhead.md),
[ADR 0018](./0018-the-trade-budget-has-three-axes.md),
[ADR 0022](./0022-a-websocket-is-a-handler-that-does-not-return.md),
...
Since ADR 0009 every `Ctx.send` has ended in `flush()`, and every WebSocket
...
What ADR 0009 wanted survives untouched. A client that does not pipeline,
...
ADR 0018's budget by a factor of twenty. Memory per idle connection reads
