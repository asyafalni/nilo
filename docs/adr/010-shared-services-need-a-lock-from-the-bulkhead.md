...
That last sentence was not true when this ADR was first written, and finding out why is part of the decision. zio's own default is **one** executor — the right default for a library that might be embedded in somebody else's thread, and the wrong one for a server process, which would otherwise leave every core but one idle. nilo now asks for one executor per core (`Options.threads`), or for one more than a container's CPU quota where there is one ([ADR 230](./230-a-cpu-quota-sets-the-thread-count.md)), which makes the concurrency real and this ADR necessary rather than merely prudent. Nothing in nilo serialises them — the request arena is per request, the fail slot is per fiber, and a Service is by definition shared. That is correct and deliberate, but it means **a Service with mutable state needs a lock, and until now nilo gave users no way to say so.**
...
Meanwhile a user cannot simply reach for `zio.Mutex`: ADR 001 exists so that nothing above `http/engine/` names zio, and that promise is worth nothing if user code has to break it to write a correct handler.
...
- **It works with no Engine underneath.** zio's mutex falls back to a plain futex when there is no fiber, so a handler that takes the lock is still callable from a unit test with no server running. That mattered more than anything else here: "a handler is an ordinary function you can test like one" is ADR 002, and a lock that only works inside a running server would have quietly taken it away.
- **`lock()` can fail, and the failure is already handled.** It returns `error.Canceled` if the request was cancelled while waiting, and `Canceled` has mapped to a 503 in the table since ADR 004. Nothing new to explain.
...
- The Bulkhead contract grows by one item. Every future Engine has to supply a lock. This is a small obligation compared to file IO (ADR 009), and unlike file IO it is needed by *every* application, not by one feature.
...
- Running on every core makes the per-fiber failure slot (ADR 006) load-bearing in a way it was not before: fibers now really do move between threads. Re-verified under load once the threads were real — **0 crossed responses out of 1,008,130**, where a thread-local would have leaked one request's message into another's response.
