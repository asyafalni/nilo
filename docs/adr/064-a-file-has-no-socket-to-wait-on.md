...
[ADR 055](./055-the-second-dialect-is-the-test-of-the-seam.md) wrote the
...
> ([ADR 053](./053-a-round-trip-is-not-the-cost-worth-chasing.md)); a SQLite
...
`build.zig` and fails the build ([ADR 038](./038-a-module-sits-where-the-loop-puts-it.md)).
...
([ADR 037](./037-a-service-that-needs-the-loop-is-finished-when-the-loop-exists.md)).
...
- **`conn: *c.sqlite3` is a public field and `pub const c` re-exports the C
  ...
  [ADR 065](./065-one-writer-is-not-a-setting-it-is-the-database.md) can
...
- **`Stmt` exposes `*c.sqlite3_stmt` with `reset`, `clearBindings` and
  `bind`.** That is what
  [ADR 051](./051-a-statement-that-is-a-constant-can-be-prepared-once.md)'s
  `plan` needs to mean anything here: a statement kept prepared on the
  connection it went down.
- **`OpenFlags.ReadOnly` and `busyTimeout(ms)`** — the two mechanisms ADR 065
  is built on.
...
Its own `Pool` is not used. It is homogeneous and ADR 065 needs one writer and
...
[ADR 027](./027-tls-is-terminated-in-front.md) rejected a TLS dependency — an
...
[ADR 013](./013-handlers-must-not-block-the-thread.md) already
watches a handler that holds its thread, so the machinery exists. Rejected as a
*decision*: it contradicts
[ADR 013](./013-handlers-must-not-block-the-thread.md) with no number behind
...
[ADR 001](./001-zio-as-the-engine-behind-the-bulkhead.md) exists to hold.
...
- **The Gate stops being password-only.** `bulkhead.Gate` gains a second
  ...
  it is a public name.
- **ADR 055's open question is answered in mechanism and left open in
