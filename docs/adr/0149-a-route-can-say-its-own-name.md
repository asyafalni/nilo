...
refusal says so rather than letting the generator find out. **Amended by
[ADR 0200](0200-a-hyphen-is-a-spelling-a-generator-can-carry.md)**: a `-` is
admitted too, because a contract generated by somebody else's tool may spell
`auth-login` and every generator carries it.
