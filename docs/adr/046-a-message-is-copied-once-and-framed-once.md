...
ADR 021 settled what a WebSocket *is* — a handler that does not return yet —
and ADR 035 settled how one connection reaches another. Neither looked at what
...
(ADR 032: this is exactly the kind of guard that would otherwise only ever be
seen passing).
...
ADR 019 says a handler that ignores the stopping flag holds the deploy open.
...
same reading ADR 021 gave a client that vanished, applied to the other
direction.
...
017 asks for, and the one the running total is kept to notice losing.
...
per connection — the number ADR 017 calls hard is untouched, and ADR 035's
...
021 promised there would not be one — "the buffer you pass is the memory, and
...
- **The Autobahn suite still does not run against this.** The close-code and
  ...
  from a failing report, which makes them the kind of guard ADR 032 is about.
