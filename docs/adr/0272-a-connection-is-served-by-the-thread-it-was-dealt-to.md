# 0272 — a connection is served by the thread it was dealt to
...
It is not an `Options` field. ADR 0002 says the Engine is not the user's
...
handler keeps too. It is not a licence to drop `nilo.Mutex` (ADR 0011):
two handlers on two threads still run at the same time.

## What it costs

Nothing on the four axes of ADR 0018 — no allocation, no per-connection
...
- [`http.md`](../../bench/result/http.md#what-a-request-costs-when-the-server-is-not-busy)
  ...
  *not* acted on: a connection quiet for longer than `idle_peek_ms`
  (200 ms) pays ~57 µs on its next request for the pages ADR 0071 gave
  back. That is the trade ADR 0071 made on purpose, and the number is now
...
- HttpArena's next run is the reading this box cannot take: `latency-10k`
  and `latency-1m`, with 4,096 in the backlog as well (ADR 0271). The
