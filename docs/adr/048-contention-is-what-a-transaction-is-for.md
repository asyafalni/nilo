...
`TimedOut` a name in [ADR 043](./043-a-deadline-needs-a-connection-you-hold.md).
...
ADR 043 documented, met from the other side: Postgres answers a failed
...
- **Binary size.** +0 stripped ReleaseFast on every example, because no example
  imports `nilo_sql` ([ADR 037](./037-a-service-that-needs-the-loop-is-finished-when-the-loop-exists.md)).
