# A handshake's signature is computed off the executor

**Status:** accepted
**Topic:** [tls](../design/tls.md)
**Applies:** [ADR 212](./212-tls-is-an-option-a-build-asks-for.md) (TLS is an option a build asks for), [ADR 044](./044-a-password-hash-is-gated-because-forgetting-is-silent.md) (a password hash runs on the blocking pool for the same reason).
...
`21890c6` plus ADR 216, against the same plus this, the arena's own entry built from each, interleaved:
