...
to say `Connection: close` ([ADR 019](./019-a-request-that-lasts-is-still-one-request.md)).
...
with its database down on purpose ([ADR 115](./115-a-boot-dials-the-connection-its-work-needs.md)):
...
honest probe is. The erased `AnyScope` ([ADR 134](./134-entropy-a-function-pointer-can-carry.md))
...
different label ([ADR 008](./008-middleware-is-an-onion-of-ctx-functions.md)): a string
