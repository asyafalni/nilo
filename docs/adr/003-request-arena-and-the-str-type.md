...
- A `Str` reached through something nilo does not walk carries no marker and is not watched: a const slice, an untagged union. The walk covers structs, optionals, arrays, mutable slices, and the active arm of a tagged union — which is how a `Str` inside a [`Patch`](./025-a-patch-needs-three-answers-and-an-optional-has-two.md) is covered.
