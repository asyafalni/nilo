# An accept loop that is out of descriptors waits

**Status:** accepted
**Topic:** [engine](../design/engine.md)
**Applies:** [ADR 017](./017-the-trade-budget-has-four-axes.md),
[ADR 159](./159-a-server-past-its-limit-says-so-at-once.md).
...
the way ADR 159's table says — it **returned from `listen()`**, ran the
...
Nothing on any axis of ADR 017. The backoff is on the error path of a loop
...
- Not held by a test in the suite: driving `accept` into `EMFILE` needs a
  ...
  the regression check, the way `bench/shutdown.py` is ADR 077's: it starts
