# 0280 — a statement that always answers answers a Row

**Status:** accepted
**Extends:** [ADR 0179](./0179-a-statement-with-a-key-in-it-has-a-single-row-answer.md)
(`rawOne` is the unwrap, not a narrower statement),
[ADR 0234](./0234-a-scalar-out-of-raw.md)

## Context

`rawOne` answers `?Row`, because a statement whose `WHERE` holds a key has
one row or none, and `?Row` is a 404 in the typed layer (ADR 0179). A
...
counted against the Row, a scalar in place of a Row (ADR 0234), the values
...
branch that is not reachable; ADR 0179 added `rawOne` over three lines
...
`rawcheck` deliberately does not do (ADR 0148); a `GROUP BY` with no groups
