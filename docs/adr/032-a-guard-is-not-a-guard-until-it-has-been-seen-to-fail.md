...
is a hard invariant (ADR 017). The test asserting it opened with a comment
...
**The build-time number that improved on its own.** ADR 026 recorded that the
...
1. **A test that fails without the guard**, checked by taking the guard away
   ...
   does for compile errors (ADR 026): each file is a program written wrong on
...
- A change adding a check adds the case that trips it. Reviewing the case is
  reviewing the check.
- The blocking detector (ADR 013) ships with seven tests, of which three
...
- ADR 026's wrong number stays in that document struck through rather than
