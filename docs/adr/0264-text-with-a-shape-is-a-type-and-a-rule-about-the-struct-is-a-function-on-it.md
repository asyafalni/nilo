# 0264 — text with a shape is a type, and a rule about the struct is a function on it

**Status:** accepted
**Extends:** [ADR 0206](./0206-a-whole-number-inside-a-range-is-a-type.md),
...
**Amends:** [ADR 0082](./0082-a-rule-of-your-own-joins-the-answer.md), which
put every rule in the handler and rejected a `nilo_rules` declaration.
...
[ADR 0205](./0205-a-body-field-that-parses-itself.md).
...
ADR 0206 already answered this for a number, and its argument is the whole
...
Two things, and the second amends ADR 0082.
...
A `Text` is a `Str` that parses itself ([ADR 0142](./0142-a-path-param-can-parse-itself.md)),
so it is read in the four places a `Str` is — a path param, a query value,
a form field, a JSON body ([ADR 0205](./0205-a-body-field-that-parses-itself.md))
...
- **`check` is a plain `fn ([]const u8) bool`**, and it is the escape hatch
  built in rather than bolted on: the rule ADR 0082 feared a language could
...
- **A `Text` never quotes the text back.** `Within` writes `not "500"`; a
  ...
  0142 wrote, `has to be <nilo_expects>, not "<text>"`. `Email` keeps the
...
- **The document says `minLength`, `maxLength` and `format`**, read off
  ...
  ([ADR 0206](./0206-a-whole-number-inside-a-range-is-a-type.md), "what was
  not done").
...
`must` here is the one ADR 0082 shipped — the same name, the same three
...
**This is what ADR 0082 rejected, and why it is not.** The rejected list
...
wrote, and its escape hatch is that it is a function. The sentence ADR 0082
...
is the size it was, which is the cost ADR 0082 kept off every binding. A
...
What ADR 0082 and ADR 0206 both rejected, and the reasoning stands: a table
...
- The roadmap loses *whether a rule like "this is an email address"
  belongs in this repository*. ADR 0082's "what was rejected" is read with
  this ADR beside it.
