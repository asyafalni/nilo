# A backlog is sized for the burst, not the load
...
Nothing on any axis of ADR 017. A backlog is a capacity; the kernel's
...
closes a connection past the cap the moment it is accepted (ADR 159's
...
- Not held by a test in the suite: a SYN drop needs a real listener and a
  ...
  ADR 194's — it opens `--conns` sockets at once, reads `ListenOverflows`
