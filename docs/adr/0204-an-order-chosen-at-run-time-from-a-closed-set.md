...
statements.* [ADR 0039](0039-the-shape-of-a-query-is-settled-while-compiling.md) is the whole of
the module's claim — the text exists before the program does — and
[ADR 0057](0057-a-statement-that-is-a-constant-can-be-prepared-once.md) is
...
the one thing ADR 0039 allows, a constant with the choice inside it:
...
field through `nilo_parse` ([ADR 0142](0142-a-path-param-can-parse-itself.md)),
...
([ADR 0205](0205-a-body-field-that-parses-itself.md)). Absent is the field's
...
built, which is ADR 0057's condition, and thirty thousand for the port's
...
and Execute on every call, the 12 µs ADR 0057 measured a name to be worth, on
a read that is usually the widest in the program. `db.raw` paid the same
until ADR 0148, and the port's CASE ladder was paying more in the planner.
...
so ADR 0057's letter allows it. Its reason does not: a cache that grows with
the requests a client chooses to send is the cache ADR 0057 refused for
...
- **Throughput: the unnamed statement, ~12 µs** at one request in flight
  (ADR 0057's figure). Not re-measured; the shape is `db.raw` before
  ADR 0148, which is where the number comes from.
