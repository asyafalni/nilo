...
**A Failure on the `Run`** the boot work is handed, for the boot's line to read. A `Run` is `nilo_core`'s and `fail` is `nilo_http`'s, so the Scope would have to name a module above it (ADR 038), and a fail function would still need the slot to find it.
