...
5. **A `Range` header that cannot be understood is ignored and the whole file goes out**, because RFC 9110 makes that a correct answer to every case; the one exception is a range past the end of the file, answered `416` with `Content-Range: bytes */<total>`. [ADR 020](../adr/020-a-range-is-a-slice-and-two-headers.md)
