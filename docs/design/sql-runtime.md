...
handler
 ...
 .in_fiber, no socket to wait on)
  ...
  ├──────────► db.watching(f): Sent{sql, plan, micros, rows, failed}, no values
  ▼
...
12. **A statement can be watched.** `db.watching(f)` is told the SQL, its plan name, how long it took, how many rows and whether it failed, for every statement in the module; the parameter values are never in it, because they are the interesting half and also a password. [ADR 108](../adr/108-a-statement-can-be-watched.md)
...
| [064](../adr/064-a-file-has-no-socket-to-wait-on.md) | SQLite's threading choice (`.hop` or `.in_fiber`) has no default, and why `sql/` cannot reach `nilo.blocking` itself |
...
- **A statement cannot say which request it came from.** `Sent` has no request id or route, so a watcher can say a statement was slow and not for which page; [ADR 108](../adr/108-a-statement-can-be-watched.md) leaves it for the same reason `fail`'s message box is fiber-bound.
