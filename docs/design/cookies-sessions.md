...
15. **The session cookie is `__Host-session` wherever the prefix can hold, and that name is read first.** With `Secure`, `Path=/` and no `Domain`, a sibling subdomain cannot plant a session of its own under a path of this site; a plain `session` from before the rename still opens and the next `set` moves it. [ADR 033](../adr/033-a-session-is-sealed-into-the-cookie.md)
