...
1. **A request nobody else on the chain would answer is refused rather than resolved.** `Content-Length` that is not `1*DIGIT`, a repeated `Content-Length` that disagrees, `Content-Length` beside chunked framing, a `Transfer-Encoding` whose final coding is not `chunked`, a folded header line, and a chunk line that ends anywhere but CRLF or carries a control byte in its extension are all refused, because guessing which reading a front end already committed to is how one request becomes two. So are a control byte anywhere in the head, a CR that does not end its line, and a method or header name that is not a token: those check every byte rather than five headers, and cost 1.5 to 4.5% of throughput end to end (ADR 231 found them). [ADR 070](../adr/070-a-request-nobody-else-would-answer-is-refused.md)
...
| ADR | What it decides |
|---|---|
| [070](../adr/070-a-request-nobody-else-would-answer-is-refused.md) | Which framing ambiguities (`Content-Length`, `Transfer-Encoding`, `Host`) and which bytes (controls, a bare CR, a name or method that is not a token) are refused rather than guessed at |
