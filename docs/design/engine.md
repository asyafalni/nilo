...
2. **A Service with mutable state locks with `nilo.Mutex`, which parks the fiber rather than the OS thread.** Handlers run concurrently across every executor thread nilo starts (`Options.threads`, one per core the process may use by default, [ADR 230](../adr/230-a-cpu-quota-sets-the-thread-count.md)), and a `std.Thread.Mutex` would stall every connection sharing that thread. [ADR 010](../adr/010-shared-services-need-a-lock-from-the-bulkhead.md)
...
12. **A connection is served, start to finish, by the executor it was dealt to.** zio built with `.scheduling = .pinned`, which `zioFor` in `build.zig` gives every copy and an application's root `zio_options` can override, trades zio's steal-on-idle for a lower CPU cost per request below saturation, and it is what lets a handler's threadlocal state read before a wait still be valid after it. [ADR 199](../adr/199-a-connection-is-served-by-the-thread-it-was-dealt-to.md)
...
16. **A container's CPU quota sets the thread count: the quota rounded up, and one more.** The affinity mask cannot see a quota, so `threads = 0` read every core of the host and the quota throttled them; the tightest limit of the process's cgroup and its parents is read at startup, and the count is logged with the reason when it is below the cores. A count past 64 is held to 64. [ADR 230](../adr/230-a-cpu-quota-sets-the-thread-count.md)
...
| [230](../adr/230-a-cpu-quota-sets-the-thread-count.md) | A container's CPU quota plus one is the default thread count; no count passes 64 |
...
- **A connection is still handed from the accepting executor to another one by round-robin `spawn`.** Serving it on the executor that accepted it would remove the last per-connection cross-thread hop, and zio can now say "here" (`spawnInto(.local)`, which a gRPC call already uses). Not taken without a run: round-robin is what spreads connections evenly today, and one acceptor per executor may not deal them as evenly by itself. In [`docs/roadmap.md`](../roadmap.md).
