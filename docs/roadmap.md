...
| `nilo_http` | whether the router needs a tree: indexing the first segment moved [ADR 017](./adr/017-the-trade-budget-has-four-axes.md)'s 10% bar out to about 40 routes, and an application with 203 exists | `zig build profile` on that application; two attempts that lost are in [`history.md`](./history.md) | the 203-route application reporting |
