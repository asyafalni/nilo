...
| `nilo_http` | a spawn homed on the executor that calls it: every zio `spawn` is dealt round-robin (`spawnTask` → `getNextExecutor`), so a connection accepted on one executor is pushed onto another's queue and that executor woken through its eventfd — the last per-connection cost left after [ADR 200](./adr/200-every-executor-accepts.md) put an acceptor on every executor. An option on spawn ("here") would also pin the socket's I/O to the accepting loop on epoll and kqueue, which is the reason zio spreads spawns at all. Multishot accept (`IORING_ACCEPT_MULTISHOT`) is the second ask from the same section, and matters only on io_uring. **The gRPC listener is where it costs most**: a fiber per call dealt to another executor is 0.77M calls a second against 3.12M run inline, and on one executor the fiber alone is 0.23 µs ([ADR 220](./adr/220-grpc-is-served-over-h2c-behind-a-flag.md)). Landed on zio `main` as `spawnInto(.local, …)` ([zio#761](https://github.com/lalinsky/zio/pull/761)) and not yet in a release; on the call fiber it is 2.7x at 256 connections ([`bench/result/http.md`](../bench/result/http.md#what-placing-a-grpc-call-on-its-own-executor-buys)). The same zio moves the scheduling mode to the root module's `zio_options`, so nilo would set `.pinned` through `b.dependency`; whether a dependent may rely on that default is asked in [#704](https://github.com/lalinsky/zio/issues/704#issuecomment-5832480560). What is left is a tag, the pin, and the call fiber on `.local` | [zio#704](https://github.com/lalinsky/zio/issues/704), open | v0.18.0, `4177579`; zio `main` at `0299e57` |
