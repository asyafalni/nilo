...
| `nilo_http` | what TLS costs a request at saturation rather than per operation: the per-request and per-connection CPU figures in [`http.md`](../bench/result/http.md) are from one instrumented client, and `wrk` over `https://` against the plain server is the throughput number ADR 0018's third axis wants and does not have yet | `zig build bench-tls-server -Dtls`, `wrk -c64 https://127.0.0.1:8787/users/1`, against `zig build run` on the same cores, interleaved pairs | a box with `wrk` or `oha` on it; the one every TLS figure so far came from has neither |
