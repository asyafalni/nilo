...
- **`docs/adr/`** — 220 binding decisions, each naming the alternative it
...
and the seven `refusals` steps check the wording of 301 error messages. Prefer making
...
zig build refusals-sql # nilo_sql's 120; also run by test-sql
...
that failed, so all 140 are re-analysed every run. They stay on `test` on
