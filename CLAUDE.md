...
- **`docs/adr/`** — 263 binding decisions, each naming the alternative it
...
and the eight `refusals` steps check the wording of 364 error messages. Prefer making
...
zig build refusals     # the framework's 158 compile-error checks — NOT the others
...
that failed, so all 158 are re-analysed every run. They stay on `test` on
