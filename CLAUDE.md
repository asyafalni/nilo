...
- **`docs/adr/`** — 214 binding decisions, each naming the alternative it
...
and the seven `refusals` steps check the wording of 276 error messages. Prefer making
...
zig build refusals     # the framework's 137 compile-error checks — NOT the others
zig build refusals-sql # nilo_sql's 100; also run by test-sql
...
zig build refusals-job # nilo_job's 12; also run by test-job
...
that failed, so all 137 are re-analysed every run. They stay on `test` on
