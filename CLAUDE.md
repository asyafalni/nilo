...
- **`docs/adr/`** — 202 binding decisions, each naming the alternative it
...
and the seven `refusals` steps check the wording of 257 error messages. Prefer making
...
zig build refusals     # the framework's 130 compile-error checks — NOT the others
zig build refusals-sql # nilo_sql's 88; also run by test-sql
...
that failed, so all 130 are re-analysed every run. They stay on `test` on
