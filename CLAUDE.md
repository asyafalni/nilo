...
- **`docs/adr/`** — 210 binding decisions, each naming the alternative it
...
and the seven `refusals` steps check the wording of 267 error messages. Prefer making
...
zig build refusals     # the framework's 136 compile-error checks — NOT the others
zig build refusals-sql # nilo_sql's 92; also run by test-sql
...
that failed, so all 136 are re-analysed every run. They stay on `test` on
