...
Three came from the port's sign-in round — a cookie, a dance with Google, a
JWKS fetch, a resolver and a permission table — and only one of them is a
feature. The other two are a character and a missing unwrap.

- **`c.routeName()` — the `operationId` of the route that matched, on the
  request**
  ([ADR 0201](./docs/adr/0201-a-middleware-can-learn-which-route-it-is-in-front-of.md)).
  A middleware is `fn(*Ctx, Next) !void` and could read the method, the path
  and the params — and not the name the route was registered under, so the
  one-middleware, one-table shape an authorisation layer wants could not be
  written. The port wrote a wrapper per registration and a test that scans its
  own source for `.named(` to make sure nothing escaped it.
  ...
  What `app.named` gave the route, or the derived `getUsersId`, **exactly as
  the API description prints it** — the derivation runs once at registration
  through the same function the document calls, so a table held against the
  document and a middleware reading the request cannot disagree about a route.
  Null for a 404, a 405 and a static file; a `HEAD` answered by the `GET` route
  carries the `GET`'s name. `app.routes().at(i).name` is the same word, for a
  test that holds a table against the route table. Nothing per request but a
  pointer store; one allocation per unnamed route at boot; eight bytes on a
  `Ctx`, which only a handler that parks its frame pays per connection.

- **`app.named("auth-login")` compiles**
  ([ADR 0200](./docs/adr/0200-a-hyphen-is-a-spelling-a-generator-can-carry.md)).
  A name is letters, digits, `_` and `-` now, still starting with a letter or
  `_`. The contract on the other side of a port was generated by a tool that
  hyphenates, OpenAPI allows it, and every generator folds it — so the check was
  refusing a spelling that protected nothing, and the port was carrying five
...
- **`sql.Bytes` can be written to Postgres, not only read from it.** The
  ...
  A Row with a `Bytes` column declared, passed the schema check and read on
  Postgres, and the first `db.insert` — or `.where`, `db.delete`, `db.raw`,
  anything that bound one — was `error.CannotBindStruct` from inside pg.zig,
  because `postgres.zig` handed the driver nilo's wrapper where `sqlite.zig`
  had been opening it to the blob zqlite wants. The cast was already right;
  the slice for it to apply to was not there. Both Wires open it now, a batch
  column of them binds as a `bytea[]`, and the live suite writes one through
  every statement shape, which is the test that would have caught it. Nothing
  to change — except deleting the `::bytea` raw statements the port wrote to
  get past it.
