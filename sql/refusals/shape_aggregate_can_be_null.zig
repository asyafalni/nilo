//! A Row grouped by nothing answers even when no row matched, and a sum
//! over no rows is null, so the field has to be able to hold one.
...
export fn refusal() void {
    _ = sql.exactlyOneFor(Totals, @TypeOf(.{}));
}
