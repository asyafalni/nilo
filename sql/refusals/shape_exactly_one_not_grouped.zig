//! `db.exactlyOne` is for the one Row that is exactly one row whatever
//! matched. A Row that is not grouped by nothing is `db.one`.
...
export fn refusal() void {
    _ = sql.exactlyOneFor(OrderCard, @TypeOf(.{}));
}
